...
```{r, echo=FALSE}
# Function to create interactions plots of all variables

interaction.plot(x.factor=BenNear_reg_l$LAG_log_TotChlA_micgL_AnnMn, trace.factor=BenNear_reg_l$Region,
                 response=BenNear_reg_l$LAG_Ttl_FWDisc_Anom_AnMn)


# y=SeagrassBm x=WaterTemp key=Region
SgBM <- ddply(BenNear_reg_l,.(LAG_log_TotChlA_micgL_AnnMn,Region), summarise, val=mean(log_mussel_Per_Cov))  # this calculates a mean EpiChla value at each binned temperature
sb  <- ggplot(data=BenNear_reg_l, aes(x=LAG_log_TotChlA_micgL_AnnMn, y=log_mussel_Per_Cov, colour=factor(Region))) + 
...
# Models
mgls1 <- gls(log_mussel_Per_Cov ~ Region+Year+LAG_UpWelAnom_anul_mn+LAG_log_TotChlA_micgL_AnnMn+
...
# mlme4 is simpler and not sig. diff. than mlme3
...
# Models
mgls12 <- gls(log_mussel_Per_Cov ~ Region+Year+WaterTmp_C_AnnMn+WaterTmp_SpringAnom+Bare_Sub_Per_Cov+ 
                                   SOf_MusselPropBmss+BLOYAdult_breed_n+Fuc_dist_Per_Cov,
              na.action=na.exclude, data=BenNear_reg_l)

# random intercept model, allows for variation between Regions
mlme12 <- lme(log_mussel_Per_Cov ~ Year+WaterTmp_C_AnnMn+WaterTmp_SpringAnom+Bare_Sub_Per_Cov+ 
                                   SOf_MusselPropBmss+BLOYAdult_breed_n+Fuc_dist_Per_Cov,
              random=~1|Region, na.action=na.exclude, data=BenNear_reg_l)
...
mlme22 <- lme(log_mussel_Per_Cov ~ Year+WaterTmp_C_AnnMn+WaterTmp_SpringAnom+Bare_Sub_Per_Cov+ 
                                   SOf_MusselPropBmss+BLOYAdult_breed_n+Fuc_dist_Per_Cov,
