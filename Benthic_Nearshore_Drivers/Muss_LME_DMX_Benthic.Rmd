...
my_interactions <- function(dataframe,x,y,key){
                   ...
                   A <- aggregate(dataframe[[y]], dataframe[key], 
                                  function(y) val=mean(y))
                     
                         
                   # make a plot
                   PT <- ggplot(data=dataframe, aes(x=x, y=y, colour=factor(key))) +
                         geom_point(data=A, aes(y=val)) + theme_bw() + 
                         geom_line(data=A, aes(y=val, group=key))
                   return(PT)
                   }
...
# Models
mgls1 <- gls(log_mussel_Per_Cov ~ Region+Year+LAG_UpWelAnom_anul_mn+
