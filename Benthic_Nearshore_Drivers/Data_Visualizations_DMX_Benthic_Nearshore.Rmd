...
library(knitr) ; library(httr) ; library(plyr) ; library(dplyr) ; library(XML) ; library(curl) ;
library(rvest) ; library(tidyr) ; library(ggplot2) ; library(stringr) ; library(psych)
...
BenNearxx <- read.csv("./BenthicNearshore_MusselQuestData.csv")
...
state <- readOGR(dsn=path.expand("./Benthic_Nearshore_Drivers"), layer="statep010")
state <- readOGR(dsn="./Benthic_Nearshore_Drivers/statep010.shp")
...
BenNear3 <- melt_BenNear2 %>%
           ...
           # dplyr::rename(DataSet=variable) %>%
            mutate_at(.funs=funs(as.character), .vars="Site_Name") %>%
            mutate_at(.funs=funs(as.character), .vars="Data_Set")

BenNear4 <- BenNear3 %>%
            filter(Data_Set %in%
                     c("Leukoma_MnBmss_gWW","Macoma_MnBmss_gWW",
                       "Saxidomus_MnBmss_gWW")) %>%
            mutate_at(.funs=funs(as.numeric), .vars="Bin_Value") %>%
            group_by(Year, Region, Site_Name) %>%
...
p16 <- ggplot(data=BenNear12, aes(x=Data_Set, y=Year)) + 
              geom_tile(aes(fill = as.factor(BinValue3)), colour = "white") +
              scale_fill_manual(breaks=c("0", "1"), values=c("gray", "green"), guide=FALSE) +
