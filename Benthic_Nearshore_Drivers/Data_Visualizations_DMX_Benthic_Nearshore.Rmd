...
# Data Visualizations for DMX Benthic Nearshore Drivers
...
Assemble the data frame by calling the data assembly script: 
```{r}
source("./Benthic_Nearshore_Drivers/Benthic_Nearshore_Data_.R")
#setwd("~/NCEAS/GoA Dynamics WG/dmx-drivers/Benthic_Nearshore_Drivers")
#BenNear <- read.csv(file="BenthicNearshore_AllData.csv", header=TRUE)
...
#B3 <- as.data.frame(BenNear2)
melt_BenNear2 <- melt(as.data.frame(BenNear2), id.vars=c("Year","Site_Name"),
                      variable_name="Data_Set")

melt_BenNear2 <- mutate(melt_BenNear2, Bin_Value = ifelse((is.na(value)),'0',
                                                   ifelse((!is.na(value)),'1',"")))




p <- ggplot(data=melt_BenNear2, aes(x=Site_Name, y=Year)) + 
