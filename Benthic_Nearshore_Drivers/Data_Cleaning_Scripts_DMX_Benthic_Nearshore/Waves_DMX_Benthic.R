...
##### Wave data from buoys from National Buoy Data Center
###########################################################
...
#############
# Wave data (annual): from National Buoy Data Center
...
Waves_3regA <- Buoys_all %>%
               ...
               summarise(WaveHgt_m_AnnMn=mean(WVHT),WavePrd_sec_AnnMn=mean(DPD),
                         WaveDir_degT_AnnMn=mean(MWD)) %>%  # get means
               ungroup() %>%
               select(Year, Region, WaveHgt_m_AnnMn, WavePrd_sec_AnnMn,
                      WaveDir_degT_AnnMn) # select waves
# copy the WPWS data for EPWS and NPWS  
WPWS <- filter(Waves_3regA, Region=="WPWS") 
...
# bind rows together
Waves_Ann <- bind_rows(Waves_3regA,NEPWS)



Waves_3regW <- Buoys_all %>%
               select(YYYY,MM,DD,hh,WVHT,DPD,MWD,BuoyID) %>%
               filter(WVHT!=99, WVHT!=999, DPD!=99, DPD!=999,
                      MWD!=99, MWD!=999) %>%  # remove missing data
               dplyr::rename(Year = YYYY) %>%       # rename column for uniformity
               ...
               group_by(Year,Region) %>%
               summarise(WaveHgt_m_Winter=mean(WVHT),WavePrd_sec_Winter=mean(DPD),
                         WaveDir_degT_Winter=mean(MWD)) %>%  # get means
               ungroup() %>%
               select(Year, Region, WaveHgt_m_Winter, WavePrd_sec_Winter,
                      WaveDir_degT_Winter) # select wind direction and speed
# copy the WPWS data for EPWS and NPWS  
WPWSW <- filter(Waves_3regW, Region=="WPWS") 
...
# bind rows together
Waves_Winter <- bind_rows(Waves_3regW,NEPWSW)
