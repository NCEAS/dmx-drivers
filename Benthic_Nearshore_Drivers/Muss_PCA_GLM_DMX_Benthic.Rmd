...
theme_boxplot <- function(base_size = 12){
  theme_bw(base_size) %+replace%
    theme(legend.key.size=unit(13,"points"),
          ...
          plot.margin=unit(c(0.75,1,0,1), "cm"), # respectively: top, right, bottom, left; refers to margin *outside* labels; default is c(1,1,0.5,0.5)
...
fig1 <- ggplot() + 
        ...
        geom_point(data=df33, aes(x=as.factor(Year), y=prob2, color=Region), shape=2, size=4,
                   position=position_jitter(w=0.3, h=0)) + 
        xlim(c("2006", "2007", "2008", "2009", "2010", "2011", "2012", 
               "2013", "2014", "2015")) + xlab("Year") + ylab("Anomoly of Mussel Percent Cover") +
...
# 
# fig1a <- ggplot() + 
#          geom_boxplot(data=BenNear_m_p, aes(x=as.factor(Year), y=mussel_Anom, color=Region)) +
...
df335_p <- df335 %>%
           ...
           full_join(BenNear_tran_l, by=c("Year","Region","Site_Name","mussel_Anom")) %>%
           select(Year, Region, Site_Name, mussel_Anom, prob5) %>%
...
BenNear_tran_l_p <- BenNear_tran_l %>%
                    ...
                    filter(!(Year %in% c(2006,2010,2012) & Region %in% c("KATM","KEFJ") &
                             Var_Val == 20))


fig23 <- ggplot() + 
         geom_boxplot(data=BenNear_tran_l_p, aes(x=as.factor(Year), y=Var_Val, 
                                                 color=Region, fill=Var_Type)) +
         xlim(c("2006", "2007", "2008", "2009", "2010", "2011", "2012", 
                                     "2013", "2014", "2015")) +
         xlab("Year") + ylab("Anomoly of Mussel Cover") + 
         theme_boxplot() + geom_hline(yintercept = 0, show.legend = FALSE) + 
