...
BenNear_reg_cpa_sub17 <- c("mussel_Anom", "log_TotChlA_micgL_AnnMn", "FWDisc_MeanYearly_ft3s1",
                           #"ENSO_anul_mn", 
                           #"NPGO_anul_mn", 
                           "UpWelAnom_anul_mn",
                           "Hobo_WaterTemp_AnnMn", 
                           "Bare_Sub_Per_Cov", 
                          # "Whelk_Sum_n_m2",
                          # "SOtt_AnnMnEngRec", 
                          # "barnacle_Per_Cov", 
...
Sce_17 <- data_sub_model_run(orig_df=BenNear_reg_cpa, select_cols=BenNear_reg_cpa_sub17)
#Sce_17
#Sce_4
```
...
```{r, echo=FALSE}
Reg_mod_list <- list(Sce_1, Sce_2, Sce_3, Sce_4, Sce_5, Sce_6, Sce_7, Sce_8, Sce_9, Sce_10,
                      Sce_11, Sce_12, Sce_13, Sce_14, Sce_15, Sce_16, Sce_17)

Reg_list_names <- c("Sce_1", "Sce_2", "Sce_3", "Sce_4", "Sce_5", "Sce_6", "Sce_7", "Sce_8", "Sce_9", 
                    "Sce_10", "Sce_11", "Sce_12", "Sce_13", "Sce_14", "Sce_15", "Sce_16", "Sce_17")
...
ggplot(data=df33, aes(prob2, mussel_Anom, color=Region)) + geom_point(size=4) + theme_bw()
cor(prob2, BenNear_reg_cpa[,38])
#cor(BenNear_reg_cpa_sub2a[,1], BenNear_reg_cpa_sub2a[,3])

# Hand calculate things   # CURRENTLY NOT THE WINNING MODEL!  NEEDS TO BE REVISED. 
...
# Plot residuals from Scenario winner
par(mfrow=c(2,2))
plot(Sce_4a)
...
##### Scenario 11
```{r, echo=FALSE, dev='pdf'}
BenNear_reg_cpa_sub2e2 <- BenNear_reg_cpa %>% select_(.dots=BenNear_reg_cpa_sub2e)
Sce_11a <- glm(mussel_Anom ~ ., data=BenNear_reg_cpa_sub2e2, family=gaussian)

prob2b <- predict(Sce_11a, BenNear_reg_cpa_sub2e2)
df33b <- cbind(BenNear_reg_cpa[,c(1,2,38)], prob2b)

ggplot(data=df33b, aes(prob2b, mussel_Anom, color=Region)) + geom_point(size=4) + theme_bw()
cor(prob2b, BenNear_reg_cpa[,38])

```
\newpage
##### Scenario 1
```{r, echo=FALSE}
BenNear_reg_cpa_sub2 <- BenNear_reg_cpa %>% select_(.dots=BenNear_reg_cpa_sub)
Sce_1a <- glm(mussel_Anom ~ ., data=BenNear_reg_cpa_sub2, family=gaussian)

prob2c <- predict(Sce_1a, BenNear_reg_cpa_sub2)
df33c <- cbind(BenNear_reg_cpa[,c(1,2,38)], prob2c)

ggplot(data=df33c, aes(prob2c, mussel_Anom, color=Region)) + geom_point(size=4) + theme_bw()
cor(prob2c, BenNear_reg_cpa[,38])

```
\newpage
##### Site winner
```{r, echo=FALSE}
BenNear_site_cpa_sub2f242 <- BenNear_reg_cpa %>% select_(.dots=BenNear_site_cpa_sub2f24)
Sce_24_s_ra <- glm(mussel_Anom ~ ., data=BenNear_site_cpa_sub2f242, family=gaussian)

prob2d <- predict(Sce_24_s_ra, BenNear_site_cpa_sub2f242)
df33d <- cbind(BenNear_reg_cpa[,c(1,2,38)], prob2d)

ggplot(data=df33d, aes(prob2d, mussel_Anom, color=Region)) + geom_point(size=4) + theme_bw()
cor(prob2d, BenNear_reg_cpa[,38])

```
\newpage
##### Transect winner
```{r, echo=FALSE}
BenNear_tran_cpa_sub2l22_r2 <- BenNear_reg_cpa %>% select_(.dots=BenNear_tran_cpa_sub2l22_r)
Sce_Sce_22_t_ra <- glm(mussel_Anom ~ ., data=BenNear_tran_cpa_sub2l22_r2, family=gaussian)

prob2e <- predict(Sce_Sce_22_t_ra, BenNear_tran_cpa_sub2l22_r2)
df33e <- cbind(BenNear_reg_cpa[,c(1,2,38)], prob2e)

ggplot(data=df33e, aes(prob2e, mussel_Anom, color=Region)) + geom_point(size=4) + theme_bw()
cor(prob2e, BenNear_reg_cpa[,38])
...
all_locs_r_p <- BenNear_reg_l %>%
                select(Year, Region) %>%
                distinct()

df33_p <- all_locs_r_p %>%
          full_join(df33, by=c("Year", "Region")) %>%
          filter(Year != "2005") %>%
          arrange(Year, Region, mussel_Anom) %>%
          mutate(prob2 = ifelse(is.na(prob2),20,prob2)) 
  
  
BenNear_reg_l_p <- BenNear_reg_l%>%
                   select(Year, Region, mussel_Anom) %>%
                   filter(Year != "2005") %>%
                   mutate(mussel_Anom = ifelse(is.na(mussel_Anom),20,mussel_Anom),
                          Var_Type = "observ") %>%
                   arrange(Year, Region, mussel_Anom) %>%
                   bind_rows(df33_p) %>%
                   mutate(Var_Type = ifelse(is.na(Var_Type),"prob","observ"),
                          Var_Val = ifelse(is.na(prob2),mussel_Anom,prob2)) 
  

fig1 <- ggplot() + 
        geom_point(data=BenNear_reg_l_p, aes(x=as.factor(Year), y=Var_Val, shape=Region, color=Var_Type), 
                   size=6, stroke=2, position=position_jitter(w=0.3, h=0)) +
        xlim(c("2006", "2007", "2008", "2009", "2010", "2011", "2012", 
               "2013", "2014", "2015")) + xlab("Year") + ylab("Anomaly of Mussel Percent Cover") +
        theme_boxplot() + geom_hline(yintercept = 0, show.legend = FALSE) + 
        coord_cartesian(ylim=c(-4,4)) +
        scale_color_manual(values=c("#98F5FF", "#FFAEB9"),
                           breaks=c("observ","prob"),
...
# 
# fig1r <- ggplot() + 
#          geom_point(data=BenNear_reg_l_p, aes(x=as.factor(Year), y=mussel_Anom, shape=Region),
#                     size=6, stroke=2, position=position_jitter(w=0.3, h=0)) +
#          xlim(c("2006", "2007", "2008", "2009", "2010", "2011", "2012", 
#                 "2013", "2014", "2015")) + xlab("Year") + ylab("Anomaly of Mussel Percent Cover") +
#          theme_boxplot() + geom_hline(yintercept = 0, show.legend = FALSE) + 
...
fig2b <- ggplot(data=line_dat_mus, aes(x=as.factor(Year), y=value, color=Variable, 
         ...
         theme(legend.position="right",
               ...
               panel.border = element_rect(fill = NA, colour = "black", size = 1))
#
# Figure of Mussel anom for OSM Portland 2018
fig2br <- ggplot(data=line_dat_mus, aes(x=as.factor(Year), y=value, color=Variable,
                                        group=as.factor(Variable))) +
          geom_line(size=5) + theme_boxplot() + xlab(NULL) +
          geom_hline(yintercept = 0, show.legend = FALSE, size=1) +
          ylab("Anomaly of Mussel % Cover  ") +
          scale_color_manual(breaks=c("mussel_Anom_mn"),
                             labels=c("  Mussels"),
                             values=c("#0072B2")) +
...
fig2e <- ggplot(data=line_dat_ens, aes(x=as.factor(Year), y=value, color=Variable, 
         ...
         geom_line(size=1) + theme_boxplot() + xlab(NULL) + 
         ylab("Index") +
         scale_color_manual(breaks=c("ENSO_anul_mn"),
...
# egg::ggarrange(plots=list(fig2b, fig2a), ncol=1, width = unit(2,"cm"), height = unit(1, "in"))
# egg::ggarrange(plots=list(fig2c, fig2e), ncol=1, width = unit(2,"cm"), height = unit(1, "in"))
# egg::ggarrange(plots=list(fig2d), ncol=1, width = unit(2,"cm"), height = unit(1, "in"))
# egg::ggarrange(plots=list(fig2f, fig2g), ncol=1, width = unit(2,"cm"), height = unit(1, "in"))

grid.draw(egg::ggarrange(plots=list(fig2b, fig2a, fig2c, fig2e, fig2d, fig2f, fig2g), 
               ncol=1, width = unit(3,"cm"), height = unit(3, "in")))
...
ggplot(data=df333, aes(prob3, mussel_Anom, color=Region)) + geom_point(size=4) + theme_bw()
...
##### Region Winner
```{r, echo=FALSE, dev='pdf'}
BenNear_reg_cpa_sub2pl2 <- BenNear_site_cpa %>% select_(.dots=BenNear_reg_cpa_sub2pl)
Sce_4_ra <- glm(mussel_Anom ~ ., data=BenNear_reg_cpa_sub2pl2, family=gaussian)

prob3a <- predict(Sce_4_ra, BenNear_reg_cpa_sub2pl2)
df333a <- cbind(BenNear_site_cpa[,c(1,2,3,38)], prob3a)

ggplot(data=df333a, aes(prob3a, mussel_Anom, color=Region)) + geom_point(size=4) + theme_bw()
cor(prob3a, BenNear_site_cpa[,38])

```
\newpage
##### Transect Winner
```{r, echo=FALSE}
BenNear_tran_cpa_sub2l22_s2 <- BenNear_site_cpa %>% select_(.dots=BenNear_tran_cpa_sub2l22_s)
Sce_22_t_sb <- glm(mussel_Anom ~ ., data=BenNear_tran_cpa_sub2l22_s2, family=gaussian)

prob3b <- predict(Sce_22_t_sb, BenNear_tran_cpa_sub2l22_s2)
df333b <- cbind(BenNear_site_cpa[,c(1,2,3,38)], prob3b)

ggplot(data=df333b, aes(prob3b, mussel_Anom, color=Year)) + geom_point(size=4) + theme_bw()
cor(prob3b, BenNear_site_cpa[,38])

```
\newpage
##### Scenario 27
```{r, echo=FALSE}
BenNear_site_cpa_sub2f272 <- BenNear_site_cpa %>% select_(.dots=BenNear_site_cpa_sub2f27)
Sce_27_sa <- glm(mussel_Anom ~ ., data=BenNear_site_cpa_sub2f272, family=gaussian)

prob3c <- predict(Sce_27_sa, BenNear_site_cpa_sub2f272)
df333c <- cbind(BenNear_site_cpa[,c(1,2,3,38)], prob3c)

ggplot(data=df333c, aes(prob3c, mussel_Anom, color=Region)) + geom_point(size=4) + theme_bw()
cor(prob3c, BenNear_site_cpa[,38])
...
fig15a <- ggplot() + 
          geom_point(data=BenNear_site_l, 
                     aes(x=as.factor(Year), y=mussel_Anom, color=Site_Name), size=4,
...
Sce_28_s_t <- data_sub_model_run(orig_df=BenNear_tran_cpa, select_cols=BenNear_site_cpa_sub2f28)
#Sce_28_s_t
Sce_28_s_t$aic

```
\newpage  
##### Scenario 22 - Best Model  
```{r, echo=FALSE}
# copy the best model outside the function so can do correlation comparison
...
ggplot(data=df335, aes(prob5, mussel_Anom, color=Site_Name)) + geom_point(size=3) + theme_bw()
...
```{r, echo=FALSE, dev='pdf'}
BenNear_reg_cpa_sub2pl_t2 <- BenNear_tran_cpa %>% select_(.dots=BenNear_reg_cpa_sub2pl_t)
Sce_4_t_ra <- glm(mussel_Anom ~ ., data=BenNear_reg_cpa_sub2pl_t2, family=gaussian)

prob5a <- predict(Sce_4_t_ra, BenNear_reg_cpa_sub2pl_t2)
df335a <- cbind(BenNear_tran_cpa[,c(1,2,3,37)], prob5a)

ggplot(data=df335a, aes(prob5a, mussel_Anom, color=Site_Name)) + geom_point(size=3) + theme_bw()
cor(prob5a, BenNear_tran_cpa[,37])

```
\newpage  
##### Site winning scenario  
```{r, echo=FALSE}
BenNear_site_cpa_sub2f282 <- BenNear_tran_cpa %>% select_(.dots=BenNear_site_cpa_sub2f28)
Sce_28_ta <- glm(mussel_Anom ~ ., data=BenNear_site_cpa_sub2f282, family=gaussian)

prob5b <- predict(Sce_28_ta, BenNear_site_cpa_sub2f282)
df335b <- cbind(BenNear_tran_cpa[,c(1,2,3,37)], prob5b)

ggplot(data=df335b, aes(prob5b, mussel_Anom, color=Site_Name)) + geom_point(size=3) + theme_bw()
cor(prob5b, BenNear_tran_cpa[,37])
...
# Figure
# BenNear_tran_l # has data through 2015
# df335$prob5 # predicted mussel data 

all_locs_p <- BenNear_tran_l %>%
              select(Year, Region) %>%
              distinct()
              
df335_p <- all_locs_p %>%
           full_join(df335, by=c("Year", "Region")) %>%
           filter(Year != "2005") %>%
           arrange(Year, Region, Site_Name, mussel_Anom) %>%
           mutate(prob5 = ifelse(is.na(prob5),20,prob5)) 
  

BenNear_tran_l_p <- BenNear_tran_l %>%
                    select(Year, Region, Site_Name, mussel_Anom) %>%
                    filter(Year != "2005") %>%
                    mutate(mussel_Anom = ifelse(is.na(mussel_Anom),20,mussel_Anom),
                           Var_Type = "observ") %>%
                    arrange(Year, Region, Site_Name, mussel_Anom) %>%
                    bind_rows(df335_p) %>%
                    mutate(Var_Type = ifelse(is.na(Var_Type),"prob","observ"),
                           Var_Val = ifelse(is.na(prob5),mussel_Anom,prob5)) 

fig23 <- ggplot() + 
         geom_boxplot(data=BenNear_tran_l_p, aes(x=as.factor(Year), y=Var_Val, 
                                                 color=Region, fill=Var_Type), lwd=1) +
         xlim(c("2006", "2007", "2008", "2009", "2010", "2011", "2012", 
                                     "2013", "2014", "2015")) +
         xlab("Year") + ylab("Anomaly of Mussel Percent Cover") + 
         theme_boxplot() + geom_hline(yintercept = 0, show.legend = FALSE) + 
         coord_cartesian(ylim=c(-4,4)) + 
         scale_color_grey() +
         scale_fill_manual(values=c("#98F5FF", "#FFAEB9"),
                           breaks=c("observ","prob"),
                           labels=c("observed","modeled")) + 
         theme(axis.text=element_text(size=15),
               axis.title.x=element_text(size=17),
...
# BenNear_tran_l_p_r <- BenNear_tran_l %>%
#                       select(Year, Region, Site_Name, mussel_Anom) %>%
#                       filter(Year != "2005") 
#                       
# 
# fig23r <- ggplot() +
#           geom_boxplot(data=BenNear_tran_l_p_r, aes(x=as.factor(Year), y=mussel_Anom), lwd=1) +
#           xlim(c("2006", "2007", "2008", "2009", "2010", "2011", "2012", "2013", "2014", "2015")) +
#           xlab("Year") + ylab("Anomaly of Mussel % Cover") +
#           theme_boxplot() + geom_hline(yintercept = 0, show.legend = FALSE) +
#           coord_cartesian(ylim=c(-4,4)) +
#           scale_color_grey() +
#           # scale_fill_manual(values=c("#98F5FF", "#FFAEB9"),
