...
BenNear_reg_cpa_sub2 <- BenNear_reg_cpa %>% #BenNear_reg_cpa_train %>%
                        select(-Year, -Region, -log_mussel_Per_Cov, #-mussel_Anom,
...
#####
BenNear_reg_cpa_sub2b <- BenNear_reg_cpa %>% #BenNear_reg_cpa_train %>%
                         select(mussel_Anom, log_TotChlA_micgL_AnnMn, SC_Mn_FWDisc_AnMn,
...
```{r, echo=FALSE}
#
prob2 <- predict(al_glm2, BenNear_reg_cpa_sub2a)
df33 <- cbind(BenNear_reg_cpa[,c(1,2,30)], prob2)

qplot(data=df33, prob2, mussel_Anom, color=Region, size=3)
cor(prob2, BenNear_reg_cpa[,30])
