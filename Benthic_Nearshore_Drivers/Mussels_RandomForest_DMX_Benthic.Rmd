...
BenNear_reg_c <- BenNear_reg %>%  filter(Year %in% c("2007","2010","2012","2013")) %>%
                 select(-mussel_Per_Cov, -Otter_Abun_est, -Otter_density_per_km2, -WndDir_degT_AnnMn,
                        -WndSp_m_s_AnnMn, -LAG_WndSp_m_s_AnnMn, -WndDir_degT_Winter, -WndSp_m_s_Winter,
                        -LAG_WndSp_m_s_Winter, -WaterTmp_SpringAnom, -LAG_Bare_Sub_Per_Cov,
                        -LAG_Fuc_dist_Per_Cov, -LED_BLOYAdult_breed_n, -LED_SOf_MusselPropBmss,
                        -SOf_MusselSumBmss_gWW, -SOf_MusselPropBmss, -log_SOf_MusselSumBmss_gWW)
...
WPWS_reg_c <- BenNear_reg %>%  filter(Year %in% c("2007","2010","2011","2012","2013"), 
                                      Region %in% c("WPWS")) %>%
              select(-mussel_Per_Cov, -Otter_Abun_est, -Otter_density_per_km2, -LAG_Fuc_dist_Per_Cov,
                     -LAG_Bare_Sub_Per_Cov, -LED_BLOYAdult_breed_n, -LED_SOf_MusselPropBmss,
                     -WaterTmp_SpringAnom)
...
KEFJ_reg_c <- BenNear_reg %>%  filter(Year %in% c(2007:2013), 
                                      Region %in% c("KEFJ")) %>%
              select(-mussel_Per_Cov, -Otter_Abun_est, -Otter_density_per_km2, -LAG_Fuc_dist_Per_Cov,
                     -LAG_Bare_Sub_Per_Cov, -WaterTmp_SpringAnom)
...
Muss_GOA <-  BenNear_GOA %>%  filter(Year %in% c(2007:2013)) %>%
             select(Year, log_mussel_Per_Cov, LAG_log_TotChlA_micgL_AnnMn, LAG_UpWelAnom_anul_mn,
...
Bio_GOA <-  BenNear_GOA %>%  filter(Year %in% c(2007:2013)) %>%
             select(Year, log_mussel_Per_Cov, WaterTmp_C_AnnMn, WaterTmp_SpringAnom, Bare_Sub_Per_Cov, 
